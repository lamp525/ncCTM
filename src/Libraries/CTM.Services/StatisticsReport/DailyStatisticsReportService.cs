...
namespace CTM.Services.StatisticsReport
{
    public partial class DailyStatisticsReportService : IDailyStatisticsReportService
    {
        ...
        private IList<UserMarginTradingInterestEntity> CalculateUserDayMarginTradingInterest(IList<InvestStatisticsCommonEntity> dailyCommonInfos, IList<DateTime> tradeDates, IList<DailyMarginTradingInfoEntity> existedDayMarginInfos)
        {
            ...
            var tradeDateByYear = tradeDates.GroupBy(x => x.Year).ToList();
            foreach (var annualTradeDates in tradeDateByYear)
            {
                ...
                decimal lastDayAnnualProfit = 0;
                int tradeDays = 0;
                foreach (var tradeDate in annualTradeDates)
                {
                   ...
                   // dailyModel.CurrentInterest = ((dailyModel.ActualFinancingAmount - lastDayAccumulatedProfit > 0 ? dailyModel.ActualFinancingAmount - lastDayAccumulatedProfit : 0) + dailyModel.ActualLoanAmount) * AppConfigHelper.MarginTradingDPR;
                    dailyModel.CurrentInterest = ((dailyModel.ActualFinancingAmount - lastDayAnnualProfit > 0 ? dailyModel.ActualFinancingAmount - lastDayAnnualProfit : 0) + dailyModel.ActualLoanAmount) * AppConfigHelper.MarginTradingDPR;
                    
                    //累计利息
                    accumulatedInterest += dailyModel.CurrentInterest;
                    dailyModel.AccumulatedInterest = accumulatedInterest;
                    dailyModel.AnnualInterest = accumulatedInterest;
                    //前一日年度收益
                    lastDayAnnualProfit = currentCommonInfo.AnnualProfit;
        ...
        private IList<UserInvestIncomeEntity> GetUserDayInvestIncomeInfo(UserInfo investorInfo, IList<string> statisticalInvestorCodes, IList<DailyRecord> dayRecords, IList<DateTime> statisticalDates, IList<TKLineToday> stockClosePrices)
        {
            ...
            foreach (var date in statisticalDates)
            {
                ...
                var incomeModel = new UserInvestIncomeEntity()
                {
                    ...
                    //年度收益
                    AnnualProfit = currentStatisticsCommonInfo.AnnualProfit,
                    //年度实际收益
                    AnnualActualProfit = currentStatisticsCommonInfo.AnnualProfit - currentMarginInterestInfo.AnnualInterest,
                    //年度利息
                    AnnualInterest = currentMarginInterestInfo.AnnualInterest,
                };
                ...
                //年度收益率（年度实际收益 / 平均融资融券额)
                incomeModel.AnnualIncomeRate = CommonHelper.CalculateRate(incomeModel.AnnualActualProfit, incomeModel.AverageMarginAmount);
        ...
        private IList<UserInvestIncomeEntity> GetUserTotalInvestIncome(UserInfo investorInfo, IList<DateTime> statisticalDates, IList<UserInvestIncomeEntity> dayInvestIncomeInfos, IList<UserInvestIncomeEntity> bandInvestIncomeInfos, IList<UserInvestIncomeEntity> targetInvestIncomeInfos)
        {
            ...
            foreach (var date in statisticalDates)
            {
                ...
                var incomeModel = new UserInvestIncomeEntity()
                {
                    ...
                    AnnualActualProfit = currentDayIncome.AnnualActualProfit + currentBandIncome.AnnualActualProfit + currentTargetIncome.AnnualActualProfit,
                    AnnualInterest = currentDayIncome.AnnualInterest + currentBandIncome.AnnualInterest + currentTargetIncome.AnnualInterest,
                    AnnualProfit = currentDayIncome.AnnualProfit + currentBandIncome.AnnualProfit + currentTargetIncome.AnnualProfit,
        ...
        {
            var commanText =  $@" EXEC [dbo].[sp_GetTradeTypeProfit] @TeamId ={teamId} ,@StartDate = '{startDate}' ,@endDate ='{endDate}'";
