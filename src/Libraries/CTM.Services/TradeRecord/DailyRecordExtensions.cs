...
namespace CTM.Services.TradeRecord
{
    public static class DailyRecordExtensions
    {
        ...
        public static IList<InvestStatisticsCommonEntity> GetDailyInvestStatisticsCommonInfo(this IList<DailyRecord> source, IList<DateTime> tradeDates, IList<TKLineToday> stockClosePrices)
        {
            ...
            decimal baseDateAccumulatedProfit = 0;
            DateTime? nextTradeDate = null;
            foreach (var date in tradeDates)
            {
                ...
                {
                    TradeDate = date,
                    DealAmount = currentRecords.Where(x => x.TradeDate == date).ToList().CalculateDealAmount(),
                    PositionValue = 0,
                    PositionProfit = 0,
                ...
                foreach (var stockGroup in recordsByStock)
                {
                    ...
                    //持仓市值
                    info.PositionValue += Math.Abs(holdingVolume) * closePrice;
                    //持仓收益
                    info.PositionProfit += holdingVolume * closePrice;
                }

                info.AccumulatedProfit = info.AccumulatedActualAmount + info.PositionProfit;
                info.AnnualProfit = info.AccumulatedProfit - baseDateAccumulatedProfit;

                nextTradeDate = tradeDates.Where(x => x > date).FirstOrDefault();
                if (nextTradeDate.HasValue && nextTradeDate.Value.Year > date.Year)
                {
                    baseDateAccumulatedProfit = info.AccumulatedProfit;
                }
